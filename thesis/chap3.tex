...
%
This method can also be extended to polynomials by treating each degree of a variable as separate column in $\mathbf{X}$, and solving for additional coefficients for each term.
%
...
%
Zou and Hastie point out several disadvantages of the LASSO method, as well.
%
...
%
For this reason, they propose the elastic net method which combines the $L_1$ and $L_2$ norms to create a regularizer that counteracts the weaknesses of both ridge and LASSO regression~\cite{ZouHastie2005}.
...
Several other methods exist for explicitly performing variable selection such as stepwise regression~\cite{DraperSmith2014}, least-angle regression (LARS)~\cite{EfronHastieJohnstone2004}, and orthogonal matching pursuit~\cite{DavisMallatZhang1994,PatiRezaiifarKrishnaprasad1993}.
%
...
\DPM{Orthogonal matching pursuit}
...
The resulting optimization of all of the described SVM methods is $O(n^2)$ with respect to the training size and the computation time is not affected by the dimensionality of the original space.
%
...
Various improvements have been made to optimize the computation costs of the support vector machines as described herein, many of these are summarized by Shawe-Taylor and Sun~\cite{Shawe-TaylorSun2011}.
%
The techniques can be broadly classified into a few categories.
%
The first category being decomposing the problem into smaller subproblems either
by chunking~\cite{BordesErtekinWeston2005,ChangLin2011,Joachims1999,OsunaFreundGirosi1997,Platt1998,Vapnik1982,ZanniSerafiniZanghirati2006} or by employing an active set strategy~\cite{CauwenberghsPoggio2000,HastieRossetTibshirani2004,Scheinberg2006,ShiltonPalaniswamiRalph2005}.
%
...
%
A second category instead looks at decomposing the problem by focusing on one
...
%
Whereas the aforementioned methods formulate a dual problem there are some methods which instead focus on solutions to the primal problem with the ultimate goal of better handling approximate solutions~\cite{}.

The remaining techniques

\DPM{Interior point ??} - \cite{RasmussenWilliams2006,ScholkopfSmola2001}

\DPM{Primal}
\begin{itemize}
  \item Newton's method
  \begin{itemize}
    \item Same time complexity as dual, but better equipped for handling approximate solutions due to the focus on minimizing an objective function
    \item \cite{Chapelle2007,KeerthiDeCoste2005,LeeMangasarian2001,Mangasarian2002}
  \end{itemize}

  \item Stochastic sub-gradient with projection
  \begin{itemize}
    \item Can handle non-differentiable objective functions
    \item \cite{KivinenSmolaWilliamson2004,Shalev-Shwartz2011,Zhang2004}
  \end{itemize}

  \item{Cutting Plane}
  \begin{itemize}
    \item ???
    \item \cite{FrancSonnenburg2008,Joachims2006,TeoSmolaVishwanathan2007}
  \end{itemize}
\end{itemize}

\subsubsection{Support Vector Regression}

Support vector machines (SVMs) for regression have proven longevity and an extensive body of research into their use and optimization.
%
...
%
The measure of loss is typically least squares, whereas in classification different loss functions are used~\cite{SmolaScholkopf2004,SuykensVandewalle1999,SuykensDeBrabanterLukas2002,DruckerBurgesKaufman1997}.
