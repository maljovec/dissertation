...
%
MDS is not restricted to linear projections, such as the Sammon mapping example stated above, and we will discuss related nonlinear efforts in Section~\ref{sec:nonlinearDR}.

\DPM{Locality preserving Projections? neighborhood preserving embedding (NPE)?} - related to nonlinear methods below.

\DPM{Distance metric learning}

Thus far, the methods we have described do not inherently support a functional relationship among the data.
%
...
%
We constrast this with another set of dimensionality reduction techniques that can be considered label- or response-aware.
...
%
The term sufficient is used to represent that the dimensionality reduction retains its predictive accuracy due to the statistical dependence constraint.
%
Surveys of these methods can be found in Adragni and Cook~\cite{AdragniCook2009} and Li~\cite{Li1999}.
%
More recently, researchers have focused on kernel mappings~\cite{FukumizuBachJordan2004,FukumizuBachJordan2009,NilssonShaJordan2007}.
\DPM{Unsupervised setting~\cite{WangShaJordan2010}}
...
% %
% Such methods are less suited for providing global sensitivity information, but may be able to provide more local sensitivity information.
...
\DPM{Locally linear embedding}

\DPM{Subspace clustering}

\DPM{Locally linear embedding}

\DPM{Laplacian Eigenmaps}
