...
%
In the case where the data is not linearly separable in the original space, we can either relax our constraint to include a soft margin that allows points to be misclassified or the data can be projected into a higher dimensional feature space using a kernel function.
...
For the case where a kernel method is used to increase the dimensionality, the same optimization process can be applied in the higher dimensional feature space in order to determine the support vectors and the maximum-margin linear separator in the feature space~\cite{BoserGuyonVapnik1992}.

\DPM{TODO: Common kernels used are...}
