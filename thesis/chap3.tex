...
\DPM{Probabilistic PCA} - 

\DPM{Kernel PCA} - Transform feature space not original data

\DPM{robust PCA} - Outlier insensitivity

\DPM{sparse PCA} - like lasso method described later in section~\ref{sec:regularization}

\DPM{nonlinear PCA} - outside of linear methods ~\cite{HyvarinenKarhunenOja2001}

\DPM{Multidimensional Scaling (MDS) - maximizes pairwise distances among points in the reduced space.
Classical MDS is equivalent to PCA.
Uses a stress function typically concerned with pairwise distances such as preserving relative distances.
Can be generalized to nonlinear mappings giving rise to techniques like Isomap.}

\DPM{Fisher's Linear Discriminant Analysis (LDA)} - Maximize separation of classes

\DPM{Canonical Correlation Analysis (CCA)}

\DPM{Independent Component Analysis}
