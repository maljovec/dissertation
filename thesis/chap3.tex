...
%
In the classical case, a hyperplane is used to divide the two classes of points where a subset of the original data is used to determine this hyperplane.
%
The specific subset of data used are known as the support vectors and can intuitively be thought of as the points closest to the boundary separating the two classes.
%
Using the support vectors, we can construct the equation of the separating hyperplane, $S(\mathbf{x}): 0 = \mathbf{w}\mathbf{x} - b$.
%
Figure~\ref{fig:svmExample} shows an annotated example of the support vector machine applied to a binary classification problem.
...
The region bounded by the separating hyperplane and the support vectors is known as the margin.
%
This sets up an optimization problem with the goal of creating a hyperplane with a maximal margin to the support vectors and with the fewest number of misclassified points (in the case where the data is not linearly separable).
%
The equation of the boundary surface, 
% $S(\mathbf{x}) \st 0 = \mathbf{w}\mathbf{x} - b$
, can be used to classify a point as belonging on the positive or negative side of


\DPM{Insert equations needed for describing optimization problem.}

% \begin{equation}
% \left[\frac{1}{n}\sum_{i=1}^{n} \max(0,1-y_i(\mathbf{w}\mathbf{x_i} - \mathbf{b}))\right] + \lambda||\mathbf{w}||^2
% \end{equation}


In the case where the data is not linearly separable in the original space, the data can be projected into a higher dimensional feature space using a kernel function.
%
Then, the same optimization process can be applied in the higher dimensional feature space in order to determine the support vectors and the maximum-margin linear separator in the feature space~\cite{BoserGuyonVapnik1992}.
