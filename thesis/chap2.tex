...
%
For example, let me begin with a term that has already been used heavily in Chapter~\ref{ch:introduction}: \emph{data}.
...
In the abstract sense, this definition is precise, however potentially somewhat unintuitive.
%
...
We will make use of 1-skeletons heavily for approximating topology in high
dimensions.
%
An important caveat to note is that the families of graphs we use do not always
result in a true 1-skeleton as we cannot guarantee that there are no crossing
edges for some parameterizations of our graphs.
%
This condition is relaxed for many of our cases as the expense to detect and
remove what amount to somewhat rare cases for points in general position in
high dimension seems to outweigh the benefits of imposing this rule strictly.
%
However, it is important to note that in general, crossing edges are not a
desirable quality for many of the analyses presented and we discuss their
presence in practice and how we can choose graphs that mitigate their appearance
in Chapter~\ref{ch:graphs}.
