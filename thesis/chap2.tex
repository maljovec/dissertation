...
\textbf{Derivative}$\mathbf{(\frac{df}{dx})}$ - Given a function $f(x)$ defined
 ...
 $\frac{df}{dx} = \lim_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h}$ when the limit exists.
 %
...
\begin{defn}
 \textbf{Level set}/\textbf{contour}/\textbf{isocontour}/\textbf{isosurface}  ($L$) -
...
\begin{figure}[b]
  ...
  $A$, and the closure of $A$ (includes points not in $A$).}
  \label{fig:sets}
\end{figure}
...
%
The boundary encompassing an $n$-manifold with boundary is itself an $n-1$-manifold without boundary.
%
