...
%
In order to do this, we first note that we require only the axes of said cones and not the interiors or boundaries.
%
...
\begin{figure}[t]
    ...
    \caption{A study of the relative areas of constrained CVT samples indicates that their error, although nonzero, is within tolerance for the examples we consider.
    %
...
%
In this setting, we assume a $k$-nearest neighbor index is available that is able to efficiently return the $k$ nearest neighbors in sorted order for one ore more query points in our dataset.
%
...
%
We accomplish this reduction by defining the edge as a directed vector, say from point $a$ to point $b$.
%
...
%
The upper limit of this shape implies that only points that have a $t$ value on the range $(0,1)$ can possibly intersect the empty region.
%
...
%
Any edge with a neighbor inside $R_{\alpha_{min}}$ will have probability zero, and any edge with no neighbors inside of $R_{\alpha_{max}}$ will have probability one.
%
...
%
For all but one of our measures, $\alpha$-shape coverage, a lower value is better.
%
...
%
However, if the entire $k$-nearest neighbor graph can fit in memory or multiple GPUs can be exploited, the FAISS-GPU algorithm may be favorable because it will not need to share GPU memory with the pruning phase.
%
...
%
The number of neighbors used in the underlying $k$-nn graph varies commensurate with the dimension, and thus performance is as much a factor of $k$ as is dimensionality in these cases.
%
...
\begin{figure}[htbp]
  ...
  \caption{A graph quality heatmap based on the aggregation of five graph quality measures on five proximity graphs for eight distributions.
  %
...
%
For instance, with the four non-space-filling distributions on the right that somehow have an intrinsic shape that is different from their embedding domain, the relaxed $\beta$-skeleton tends to do well with the setting $\beta=1.5$ and $p=2$, whereas the yao and $\Theta$ graphs tend to favor $m=1$ for those cases.
%
...
%
This study indicates that when operating on non-simply-connected domains, the difference between the strict and relaxed $\beta$-skeleton is indeed significant.
%
...
\begin{figure}[htbp]
    ...
    \caption{Analysis of five graphs using our graph quality measures.
    %
...
\begin{figure}[htbp]
    ...
    \caption{Examples of the best performing strict $\beta$-skeleton are shown for each distribution under consideration.}
...
\begin{figure}[htbp]
    ...
    \caption{Examples of the best performing relaxed $\beta$-skeleton are shown for each distribution under consideration.}
...
%
Higher values are preferred becuase they indicate that the graph is able to report the ``correct'' partition over a larger range of persistence values.
%
...
%
The functional form of the chosen class of functions is sufficiently general to generate a wide variety of reponse surfaces.
%
...
\begin{figure}[htbp]
    ...
    \caption{Visual representations of our three test functions in two dimensions.}
    \label{fig:datasets}
\end{figure}
...
%
The summarized results are presented in Figure~\ref{fig:graph_topo} for three examples in four and five dimensions.
%
...
\begin{figure}[htbp]
    ...
    \caption{Analysis of five graphs under the topological setting for various test functions in varying dimensions using various sample sizes.
...
%
The dimensionalities of the two examples are 16 and 12, respectively, and each is made up of roughly 20000 point samples.
...
\begin{figure}[t]
    ...
    \caption{Analysis of five graphs for two classification datasets.
    %
