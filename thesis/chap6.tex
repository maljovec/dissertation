...
\DPM{conference paper~\cite{MaljovecWangRosen2016}}
...
\subsubsection{Design Requirement A: Structure-based domain partitioning amenable to local SA.}
...
\subsubsection{Design Requirement E: Scalable analysis and visualization.}
...
\subsubsection{Sensitivity View}
...
%
\DPM{Insert formulation for sensitivity coefficients}
%
...
%
For an $m$-dimensional dataset, we will end up with $m$ different linear fits and for each fit, we compute the $R^2$ score given in \DPM{Section~\ref{sec:background}}.
%
...
\subsubsection{Secondary Visual Encodings}
...
\begin{figure}[!h]
  ...
  (b) Scatterplot projecting the most significant input \DPM{Fix error here}}
  %$\powerf$, $\grainf$, and the output $\maxs$. (c) Linear coefficients. }
